...
public class ClickPostHairStylingNakuru extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_click_post_gel_nails_nakuru);

        mAuth = FirebaseAuth.getInstance();
        currentUserID = mAuth.getCurrentUser().getUid();

        PostKey = getIntent().getExtras().get("PostKey").toString();
        ClickPostRef = FirebaseDatabase.getInstance().getReference().child("Services").child("Nakuru").child("HairStyling").child(PostKey);
        ...
        call.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view)
            {
                ...
                if (ContextCompat.checkSelfPermission(ClickPostHairStylingNakuru.this,
                        Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(ClickPostHairStylingNakuru.this, new String[]{Manifest.permission.CALL_PHONE},1);

                    Toast.makeText(ClickPostHairStylingNakuru.this, "Please wait while we are calling"+servicePhoneNumber+"", Toast.LENGTH_LONG).show();
                }
                else
                {
